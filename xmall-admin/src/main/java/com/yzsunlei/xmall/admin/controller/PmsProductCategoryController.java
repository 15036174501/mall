...
public class PmsProductCategoryController {
    ...
    @ApiOperation("根据level查询商品分类")
    @RequestMapping(value = "/listWithRoot", method = RequestMethod.GET)
    @ResponseBody
    public Object getRootList() {
        List<PmsProductCategoryExpand> list = productCategoryService.getRootList();
        if(list.size()>0){
            for (int i=0;i<list.size();i++){
                PmsProductCategoryExpand model=list.get(i);
                model.setIsUse(true);
                model.setPaixu(Long.parseLong(i+""));
            }
        }
        return new CommonResult().success(list);
    }
