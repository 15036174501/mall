...
public class SmsHomeAdvertiseController {
    ...
    @ApiOperation("轮播图查询")
    @RequestMapping(value = "/listHomeAdvertise", method = RequestMethod.GET)
    @ResponseBody
    public Object advertiseList(@RequestParam(value = "type", required = false) Integer type) {
        List<SmsHomeAdvertiseExpand> advertiseList = advertiseService.listHomeAdvertise(type);
        if(advertiseList.size()>0){
            for (int i=0;i<advertiseList.size();i++){
                SmsHomeAdvertiseExpand model=advertiseList.get(i);
                int status=model.getStatus();
                if(status==0){
                    model.setStatus(1);
                }else if(status==1){
                    model.setStatus(0);
                    model.setStatusStr("显示");
                }
            }
        }
